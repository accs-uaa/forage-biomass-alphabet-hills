...
raw_file = paste(data_folder,
                 ...
                 round_date,
                 'physiography/confusion_matrix_raw.csv',
                 sep = '/')
...
output_file = paste(data_folder,
                    'Data_Output/model_results',
                    round_date,
                    'physiography/confusion_matrix.csv',
                    sep = '/')
...
confusion_matrix = raw_data %>%
  rename(burned = X1, barren = X2, drainage = X3, riparian = X4,
         floodplain = X5, water = X6, uplandlowland = X7, aspen = X8) %>%
  mutate(Actual = case_when(Actual == 1 ~ 'burned',
                            Actual == 2 ~ 'barren',
                            Actual == 3 ~ 'drainage',
                            Actual == 4 ~ 'riparian',
                            Actual == 5 ~ 'floodplain',
